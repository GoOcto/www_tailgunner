...
export class StarField {
    ...
    update() {
        const cx = paper.view.size.width * .5;
        const cy = paper.view.size.height * .5;
        for (let i = 0; i < this.stars.length; i++) {
            let star = this.stars[i];
            star.z += this.speed;
            if (star.z > this.maxDistance) { star.shape.remove(); this.stars[i] = this.spawnStar(false); star = this.stars[i]; }
            const scale = this.fov / (this.fov + star.z);
            const sx = cx + star.x * scale;
            const sy = cy + star.y * scale;
            const radius = Math.max(.1, star.baseRadius * scale);
